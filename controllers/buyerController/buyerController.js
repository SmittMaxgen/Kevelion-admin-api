...
//     await pool.query(
//       `UPDATE buyer SET name=?, mobile=?, email=?, image=?, address=?, status=?, address_id=?, approve_status=?, is_online=?, device_token=? , gst_no=? , fcm_token=? , WHERE id=?`,
//       [
//         name || currentBuyer.name,
//         mobile || currentBuyer.mobile,
//         email || currentBuyer.email,
...
//         fcm_token || currentBuyer.fcm_token,
//         id,
//       ],
//     );
...
export const updateBuyer = async (req, res) => {
  try {
    ...
    // ✅ Fixed: removed stray comma before WHERE
    await pool.query(
      `UPDATE buyer SET 
        name=?, mobile=?, email=?, image=?, address=?, 
        status=?, address_id=?, approve_status=?, 
        is_online=?, device_token=?, gst_no=?, fcm_token=? 
       WHERE id=?`,
      [
        name || currentBuyer.name,
        mobile || currentBuyer.mobile,
    ...
    if (companyRows.length > 0) {
      await pool.query(
        `UPDATE buyer_company_details SET 
          company_name=?, company_GST_number=?, company_website=?, 
          IEC_code=?, annual_turnover=?, facebook_link=?, 
    ...
    if (kycRows.length > 0) {
      await pool.query(
        `UPDATE buyer_kyc_details SET 
          aadhar_number=?, driving_license_number=?, driving_license_dob=?, 
